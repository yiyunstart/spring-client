...
public interface CameraMapper extends BaseMapper<Camera> {
    ...
    @Select("select * from t_camera where ip=#{ip}")
    public Camera selectByIp(@Param("ip") Object ip);


    @Update("update t_camera set rows=#{rows},cols=#{cols} where ip=#{ip}")
    public void  updateGrid(@Param("ip") String ip,@Param("rows")  int rows, @Param("cols") int cols);
}
